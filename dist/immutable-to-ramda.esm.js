...
import { types, visit, print, parse } from 'recast';
import { isEmpty, difference, fromPairs, propEq, pluck } from 'ramda';
...
var getIn = function getIn(_ref2) {
  ...
  var allLiteral = function allLiteral(x) {
    return x.every(propEq('type', 'StringLiteral'));
  };
  if (!isEmpty(defValue)) {
  ...
  var expr = propsArray.type === 'ArrayExpression' && allLiteral(elements) ? pluck('value', elements).map(identifier).reduce(function (acc, curr) {
...
var nameAndTransform = function nameAndTransform(ts, immutableRamdamap) {
  return fromPairs(Object.entries(immutableRamdamap).map(function (_ref7) {
...
function addImport(path) {
  ...
  var newImportsIdentifiers = difference(Array.from(functionsToImport), importsIdentifiers);
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            return _context.abrupt("return", promises.readFile(path, 'utf-8').then(function (src) {
              return {
                src: src,
                ast: parse(src, {
                  parser: {
    ...
    return _regeneratorRuntime().wrap(function _callee3$(_context3) {
      while (1) {
        switch (_context3.prev = _context3.next) {
          ...
          case 5:
            diffLines(code.src, code.newSrc).forEach(function (part) {
...
program.option('--dry');
var args = /*#__PURE__*/program.parse(process.argv);
args.args.forEach(program.opts().dry ? printDiff : rewriteSource);
//# sourceMappingURL=immutable-to-ramda.esm.js.map
